<#
.SYNOPSIS
    SaveVault -- "git for save data" command line interface.
.DESCRIPTION
...
.EXAMPLE
    .\savevault.ps1 init
    .\savevault.ps1 watch -Known
    .\savevault.ps1 commit -m "beat the Elite Four"
    .\savevault.ps1 log
    .\savevault.ps1 diff abc123 def456
    .\savevault.ps1 restore abc123
#>
param(
...
function Show-Usage {
    Write-Host ""
    Write-Host "  SaveVault -- git for save data" -ForegroundColor Cyan
    Write-Host "  ================================================================" -ForegroundColor DarkGray
    Write-Host "  .\savevault.ps1 init                  Initialize a vault here" -ForegroundColor White
    Write-Host "  .\savevault.ps1 watch -Known          Auto-watch emulator save dirs" -ForegroundColor White
    Write-Host "  .\savevault.ps1 watch -Add <dir>      Watch an explicit directory" -ForegroundColor White
    Write-Host "  .\savevault.ps1 commit -m ""msg""       Snapshot current saves" -ForegroundColor White
    Write-Host "  .\savevault.ps1 status                Diff working tree vs HEAD" -ForegroundColor White
    Write-Host "  .\savevault.ps1 log                   Show commit history" -ForegroundColor White
    Write-Host "  .\savevault.ps1 diff <a> <b>          Compare two commits" -ForegroundColor White
    Write-Host "  .\savevault.ps1 restore <ref>         Restore a commit (auto-backs up first)" -ForegroundColor White
    Write-Host ""
...
switch ($Command.ToLower()) {
    "init" {
        Initialize-SaveVault -Path $VaultRoot | Out-Null
    }
    "watch" {
        $Paths = Get-VaultPaths -VaultRoot $VaultRoot
        if (-not (Test-Path $Paths.Dir)) { Write-Host "  [FAIL] Not a SaveVault. Run 'init' first." -ForegroundColor Red; Exit 1 }
        if ($Known) {
    ...
    "diff" {
        # diff takes two positional refs: .\savevault.ps1 diff <a> <b>
        # $Path was consumed as vault root, so read raw args instead.
        $RawArgs = @($args)
        if ($RawArgs.Count -lt 2) { Write-Host "  [FAIL] Usage: .\savevault.ps1 diff <commit-a> <commit-b>" -ForegroundColor Red; Exit 1 }
        $Paths = Get-VaultPaths -VaultRoot $VaultRoot
        Compare-SaveCommit -Paths $Paths -FromRef $RawArgs[0] -ToRef $RawArgs[1]
    }
    "restore" {
        $RawArgs = @($args)
        if ($RawArgs.Count -lt 1) { Write-Host "  [FAIL] Usage: .\savevault.ps1 restore <commit>" -ForegroundColor Red; Exit 1 }
        if (-not $Force) {
