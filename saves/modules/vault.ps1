# ==============================================================================
# SaveVault -- "git for save data" engine
# ==============================================================================
...
function Get-VaultPaths {
    ...
    $sv = Join-Path $VaultRoot ".savevault"
    return @{
        Root      = $VaultRoot
        Dir       = $sv
...
function Initialize-SaveVault {
    param([Parameter(Mandatory = $true)][string]$Path)
    $Paths = Get-VaultPaths -VaultRoot $Path
    if (Test-Path $Paths.Dir) {
        Write-Host "  [i] SaveVault already initialized at: $($Paths.Dir)" -ForegroundColor DarkGray
        return $Paths
    }
    ...
    Write-Host "  [OK] SaveVault initialized at: $($Paths.Dir)" -ForegroundColor Green
    return $Paths
}
...
function Add-SaveWatchPath {
    ...
    if (-not $Config) { Write-Host "  [FAIL] Not a SaveVault. Run 'init' first." -ForegroundColor Red; return $false }
...
function New-SaveCommit {
    ...
    $Config = Read-VaultConfig -Paths $Paths
    if (-not $Config) { Write-Host "  [FAIL] Not a SaveVault. Run 'init' first." -ForegroundColor Red; return $null }
