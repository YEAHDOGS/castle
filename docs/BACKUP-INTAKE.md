...
physical disk → share mapping is still user's call (marked below).
...
.castle-backups/
  ...
  machines/
    <machine-name>/     # e.g. user-laptop, user-desktop
...
ssh castle 'python3 ~/castle/backup/intake.py register \
  --machine user-laptop --kind disk-image \
...
## Transport — still needs user's answer ❓

| question | status |
|---|---|
| Physical target: which device/volume backs `~/.castle-backups` (the 10TB drive)? | **needs user** |
| SMB share name exposing it to the LAN (Veeam needs this in Step 4.4) | **needs user** |
| Credentials for the share / SSH user for the intake CLI | **needs user** |
| Veeam Agent Free job definition once the share exists | open Phoenix-side work |

Proposed (only a proposal until user says so): SMB share `castle-backups`
