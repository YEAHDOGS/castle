...
def t_register_disk_image():
    ...
    rec = intake.register(root, "user-laptop", "disk-image",
                          "QUARANTINE-INFECTED-2026-09-09", f, _sha(f))
    assert rec["relpath"].startswith("machines/user-laptop/images/")
...
def t_register_data_goes_to_data_dir():
    ...
    rec = intake.register(root, "user-laptop", "data", "DATA-2026-09-09",
                          f, _sha(f))
    assert rec["relpath"].startswith("machines/user-laptop/data/")
...
def t_sha_mismatch_refuses_and_writes_nothing():
    ...
    try:
        intake.register(root, "user-laptop", "disk-image", "LBL", f, "0" * 64)
